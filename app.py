...
def start_minecraft_bot():
    ...
    if not server_address or not username:
        logging.error("MC_SERVER_ADDRESS and MC_USERNAME env vars are required. Bot not starting.")
        return

    factory = AFKBotFactory()
    factory.profile.display_name = username
    reactor.connectTCP(server_address, server_port, factory)
    logging.info("Minecraft bot connection process initiated.")
...
if not reactor.running:
    ...
    logging.info("Twisted reactor started in a background thread.")
